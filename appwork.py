...
st.markdown(
    ...
    /* BANNER HERO CON NUOVA IMMAGINE DI SFONDO */
    .hero-full-bleed { 
        width: 100vw; 
        position: relative; 
        left: 50%; 
        ...
        margin-left: -50vw; 
        margin-right: -50vw; 
        background-image: linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.8)), url('https://images.unsplash.com/photo-1550966871-3ed3cdb5ed0c?w=1800'); 
    ...
    /* LOGO: flashjob con fulmine vicino e lettere ravvicinate */
    .brand-title { 
        color: white; 
        font-size: 3.5rem; 
        font-weight: 900; 
...
# LOGO AGGIORNATO CON FLASHJOB E FULMINE VICINO
st.markdown(
    """
...
if scelta == "🏠 Chi Siamo & Atmosfera":
  ...
  st.markdown(
      "<p style='color: #cbd5e1;'><b>flashjob⚡</b> nasce per sradicare il caos"
  ...
  with col_v1:
    st.markdown(
        """<div class="feature-card"><span class="badge-tag">PER LE AZIENDE 🏢</span><h4 style="color: #fff;">Perché sceglierci</h4><ul style="color: #cbd5e1;"><li>Zero commissioni</li><li>Contatto diretto WhatsApp</li><li>Copertura turni in 2 min</li></ul></div>""",
        unsafe_allow_html=True,
    )
  with col_v2:
    st.markdown(
        """<div class="feature-card"><span class="badge-tag" style="background:#bbf7d0;color:#166534;">PER I LAVORATORI 👤</span><h4 style="color: #fff;">I tuoi vantaggi</h4><ul style="color: #cbd5e1;"><li>100% Gratuito</li><li>Vetrina d'eccellenza</li><li>Zero intermediari</li></ul></div>""",
        unsafe_allow_html=True,
    )
  st.markdown("</div>", unsafe_allow_html=True)
  ...
  for lav in st.session_state.lavoratori:
    st.markdown(
        f"""<div class="feature-card"><h4 style="color: white; margin-top:0;">👤 {lav['nome']}</h4><p style="color: #cbd5e1;">Mansione: {lav['mansione']} | Zona: {lav['zona']}</p><p>📞 <a href="https://wa.me/{lav['tel'].replace(' ', '')}" target="_blank" style="color: #4ade80;">{lav['tel']}</a></p></div>""",
        unsafe_allow_html=True,
    )
  st.markdown("</div>", unsafe_allow_html=True)
